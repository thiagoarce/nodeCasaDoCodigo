...
function getRootDeps(template, context) {
    if (production && template.at_) {
        return template.at_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template.at_ = Object.keys(template.au_).map(key => template.au_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target.au_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag.au_) Object.assign(deps, tag.au_);
